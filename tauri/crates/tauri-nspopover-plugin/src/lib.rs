...
//     fn to_popover(&self, options: ToPopoverOptions) {
//         let tray = self.app_handle().tray_by_id(self.label()).unwrap();
...
impl<R: Runtime> AppExt<R> for AppHandle<R> {
    ...
    fn show_popover(&self) {
        ...
        // 🌟 FIX: Use standard NSPopover visibility checking methods
        if unsafe { !popover.isShown() } {
            unsafe {
                // To display an actual NSPopover, we must give it a target frame edge.
                // We pull the main application's primary window view hierarchy to anchor it.
                if let Some(main_window) = self.get_webview_window("main") {
                    let ns_window_ptr = main_window.ns_window().unwrap();
                    let ns_window = (ns_window_ptr.cast() as *mut objc2_app_kit::NSWindow)
                        .as_ref()
                        .unwrap();
                    let anchor_view = ns_window.contentView().unwrap();
                    let anchor_rect = anchor_view.bounds();

                    // 🌟 Safely present the popover relative to our window frame boundary
                    popover.showRelativeToRect_ofView_preferredEdge(
                        anchor_rect,
                        &anchor_view,
                        objc2_foundation::NSRectEdge::MaxY, // Anchors to the bottom edge
                    );
                } else {
                    // Fallback fallback selector if no explicit anchor view is easily accessible:
                    // If you intended this to be a completely free-floating window frame,
                    // your popover.rs MUST return an NSPanel instead of an NSPopover!
                    eprintln!("⚠️ Cannot show popover: Core anchor window reference missing.");
                }
            }
        }
    }
