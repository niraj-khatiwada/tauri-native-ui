...
class TooltipManager {
    ...
    func show(text: String, keys: [String], minX: Double, minY: Double) {
        ...
        let finalRect = calculatePanelFrame(parentWindow: parentWindow, size: requiredSize, minX: minX, minY: minY)
    ...
    private func repositionPanel(_ panel: NSPanel, minX: Double, minY: Double) {
        guard let parentWindow = NSApplication.shared.keyWindow ?? NSApplication.shared.windows.first,
    ...
    private func calculatePanelFrame(parentWindow: NSWindow, size: NSSize, minX: Double, minY: Double) -> NSRect {
        let windowFrame = parentWindow.frame
        
        // Translate the target X/Y directly into global screen space
        let componentScreenX = windowFrame.origin.x + CGFloat(minX)
        let componentScreenY = (windowFrame.origin.y + windowFrame.height) - CGFloat(minY)
        
        // Center the tooltip over the direct point coordinates passed
        let panelX = componentScreenX - (size.width / 2)
        // Position it 12 pixels directly above the hover coordinate line
        let panelY = componentScreenY + 12
...
class TooltipViewController: NSViewController {
    ...
    private func buildContent() {
        ...
        for key in keys where !key.isEmpty {
            ...
            capContainer.layer?.backgroundColor = NSColor(white: 1.0, alpha: 0.12).cgColor // Slightly lighter backdrop fill
            ...
            NSLayoutConstraint.activate([
                ...
                capLabel.trailingAnchor.constraint(equalTo: capContainer.trailingAnchor, constant: -5),
...
@_cdecl("hide_native_tooltip")
public func hideNativeTooltip() {
    DispatchQueue.main.async {
